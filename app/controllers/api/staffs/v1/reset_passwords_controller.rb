class Api::Staffs::V1::ResetPasswordsController < Api::StaffsController
  ...
  def create
    staff = Staff.where("lower(email) = ?", params[:email].try(:downcase).try(:strip)).first
    if staff.present?
      staff.send_reset_password_instructions
      set_response(message: "Reset password instruction has been sent to your email '#{staff.email}'")
    else
      set_response(message: "User not found", status: 400)
    end
  end

  def edit
    @staff = Staff.with_reset_password_token(params[:token])
    if @staff.present?
      if @staff.reset_password_period_valid?
      ...
      set_response(message: "Staff not found", status: 400)
    end
  end
  ...
  def update
    ...
    if current_staff.update(update_password_params)
      current_device.update(reset_password: false, status: "signout")
      sign_out(:staff)
      set_response(message: "Successfully reset your password")
    else
      set_response(message: "Error: #{current_staff.errors.full_messages.join(" and ")}")
    end
  end
  ...
  def update_password_params
    begin
      params.require(:staff).permit(:password, :password_confirmation).merge({ reset_password: true })
